...
#  $DOCKER_COMPOSE_COMMAND up -d caddy zitadel
  ...
  # required for TURN server and Cloudpanel UFW firewall
  ufw allow 3478/udp
  ...
  cat <<EOF
{
    ...
    "Relay": {
        "Addresses": ["$NETBIRD_RELAY_PROTO://$NETBIRD_DOMAIN:$NETBIRD_PORT/relay"],
        "CredentialsTTL": "24h",
...
NB_EXPOSED_ADDRESS=$NETBIRD_RELAY_PROTO://$NETBIRD_DOMAIN:$NETBIRD_PORT/relay
